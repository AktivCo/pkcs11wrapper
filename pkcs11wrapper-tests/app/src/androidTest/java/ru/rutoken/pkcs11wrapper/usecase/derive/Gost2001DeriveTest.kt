...
class Gost2001DeriveTest {
    @Test
    fun deriveKey() {
        assumeTrue(token.value.isMechanismSupported(CKM_GOSTR3410_DERIVE))

        val recipientPublicKey = recipientKeyPair.value.publicKey
            .getByteArrayAttributeValue(session.value, CKA_VALUE).byteArrayValue

        val params = CkGostR3410DeriveParams(CKD_CPDIVERSIFY_KDF_JRT.asLong, recipientPublicKey, UKM)
        val mechanism = Pkcs11Mechanism.make(CKM_GOSTR3410_DERIVE, params)
    ...
    companion object {
        ...
        private val senderKeyPair = attributeFactory.makeGostR3410_2001KeyPairRule(session)
        private val recipientKeyPair = attributeFactory.makeGostR3410_2001KeyPairRule(
            session,
            "$TEST_2001_PUBLIC_KEY_LABEL recipient",
            "$TEST_2001_PRIVATE_KEY_LABEL recipient"
        )
