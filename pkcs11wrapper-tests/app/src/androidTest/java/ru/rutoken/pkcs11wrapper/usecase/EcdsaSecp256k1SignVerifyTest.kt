...
import ru.rutoken.pkcs11wrapper.constant.standard.Pkcs11MechanismType.CKM_ECDSA_SHA256
import ru.rutoken.pkcs11wrapper.constant.standard.Pkcs11UserType
import ru.rutoken.pkcs11wrapper.main.*
import ru.rutoken.pkcs11wrapper.mechanism.Pkcs11Mechanism
import ru.rutoken.pkcs11wrapper.rule.highlevel.*

class EcdsaSecp256k1SignVerifyTest {
    @Test
    fun signVerify() {
        with(session.value) {
            assumeTrue(token.isMechanismSupported(CKM_ECDSA_SHA256))
            val mechanism = Pkcs11Mechanism.make(CKM_ECDSA_SHA256)
    ...
    companion object {
        ...
        private val keyPair = module.value.attributeFactory.makeEcdsaKeyPairRule(session, EC_PARAMS_SECP256K1_OID)
